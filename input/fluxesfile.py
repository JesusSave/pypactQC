...
from pypactQC.util.numerical import get_float, is_float
from pypactQC.util.exceptions import PypactException, PypactOutOfRangeException, PypactDeserializeException
...
class FluxesFile(JSONSerializable):
    ...
    def _deserialize(self, f):
        ...
        if group not in ALL_GROUPS:
            raise PypactDeserializeException("Group structure {} not known to pypactQC.".format(group))
