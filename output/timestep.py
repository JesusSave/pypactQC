...
from pypactQC.output.tags import TIME_STEP_HEADER
...
class TimeStep(JSONSerializable):
    ...
    def __init__(self, ignorenuclides=False):
        ...
        self.total_gamma_dose_rate = 0.0
        self.total_gamma_dose_rate_error = 0.0
        self.nuclides = []
        self.nuclidesUE = []
    ...
    def fispact_deserialize(self, filerecord, interval):
        ...
        if substring:
            ...
            self.total_heat_error = pf.last(datadump=substring,
                                headertag='Total Heat Production is',
                                starttag='Error is',
                                endtag='% of the total.',
                                ignores=TIME_STEP_IGNORES,
                                asstring=False)
            ...
            self.total_activity_error = pf.last(datadump=substring,
                                headertag='Total Activity is',
                                starttag='Error is',
                                endtag='% of the total.',
                                ignores=TIME_STEP_IGNORES,
                                asstring=False)
            self.total_gamma_dose_rate = get_value(starttag='Total Gamma Dose Rate is', endtag='+/-')
            self.total_gamma_dose_rate_error = pf.last(datadump=substring,
                                headertag='Total Gamma Dose Rate is',
                                starttag='Error is',
                                endtag='% of the total.',
                                ignores=TIME_STEP_IGNORES,
                                asstring=False)
